...
public class ShiftServiceImpl implements ShiftService {
    ...
    public List<Color> getColors() {
        ...
        if (colors.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Data Warna tidak di temukan");
        }

        return colors.stream()
                .filter(color ->
                        !color.getName().equalsIgnoreCase("red"))
                .toList();
    }
    ...
    public ShiftResponse add(CreateShiftRequest request) {
        ...
        Color color = colorRepository.findById(request.getColorId()).orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Data Warna tidak di temukan")
        );
